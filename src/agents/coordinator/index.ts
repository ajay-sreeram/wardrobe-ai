...
function recentConversationContext(messages: ChatMessage[]) {
  ...
  const contextEntries = messages.slice(boundaryIndex + 1).flatMap((message) => {
    ...
    if (message.kind === 'duplicate') return [`${time} Photo finding awaiting review: observed ${message.garmentName}; suggested wardrobe match ${message.existingGarmentName} [${message.existingGarmentId}]${message.wearContext ? `; reported worn ${message.wearContext.wornAt}` : ''}.`];
