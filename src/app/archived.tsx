...
export default function ArchivedPiecesScreen() {
  ...
  const loadGarments = useCallback(async () => {
    ...
    setError(null);
    try {
      setGarments(await readArchivedWardrobeCatalog(db));
    } catch {
      setError('Archived pieces could not be read from this device. Your saved data has not been changed.');
    } finally {
      setLoading(false);
    }
  }, [db]);
  ...
  useEffect(() => {
    ...
    readArchivedWardrobeCatalog(db)
      ...
      .catch(() => { if (active) setError('Archived pieces could not be read from this device. Your saved data has not been changed.'); })
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {(loading || error) && !garments.length ? (
        <DataLoadState error={error} label="Loading archived pieces…" loading={loading} onRetry={() => { void loadGarments(); }} />
      ) : (
